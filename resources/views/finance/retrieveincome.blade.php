...
<div class="background">
    ...
    <div class="container">
        <div class="row">
        <h2 style="text-align:center"><u>View Income Details</u></h2>
        <br><br><br><br>
            <div class ="col-md-20">
                ...
                <table class="table table-primary table-striped">
                    ...
                    <tbody>
                        @foreach ($data as $income)
                            <tr>
                                <td>{{$income -> income_refno}}</td>
                                <td>{{$income -> transaction_date}}</td>
                                <td>{{$income -> income_amount}}</td>
                                <td>{{$income -> income_category}}</td>
                                <td>{{$income -> income_discount}}</td>
                                <td>{{$income -> income_description}}</td>
                                
                                <td>
                                    <a href="/updateincome/{{$income ->invoice_number}}" class="btn btn-info">Update</a>
                                    <a href="/deleteincome/{{$income->invoice_number}}" class="btn btn-info" onclick= "return confirm('Do you really want to delete?')">Delete</a>
                                </td>
