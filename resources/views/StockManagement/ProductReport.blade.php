    ...
    <div id="report-content">
        ...
        @foreach ($weekData as $weekData)
            <ol style="margin-top: 3px;">
                <li>Product Name: {{ $weekData->productName }}</li>
                <li>Description: {{ $weekData->descriptions }}</li>
                <li>Quantity: {{ $weekData->quantity }}</li>
                <li>Price Per Unit: {{ $weekData->pricePerUnit }}</li>
                <li>Total Amount: {{ $totalAmount = $weekData->quantity * $weekData->pricePerUnit }}</li>
            </ol>
            <hr><br>
        @endforeach
    </div>
